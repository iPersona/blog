use crate::{AppState, LoginUser, ArticleList, UserInfo, Comments, ArticlesWithTag, RegisteredUser};
use actix_web::{Error, HttpRequest, HttpResponse, Json, Form, Path, App};
use futures::Future;
use crate::api::{ApiResult, JsonResponse, InnerContext};
use log::info;
use crate::models::articles::{ArticleSlice, QuerySlice, CommentsResponse, ViewArticle, ListAllArticleFilterByTag, ListComments};
use uuid::Uuid;
...
//    }
    pub fn list_all_article(req: &HttpRequest<AppState>) -> JsonResponse {
        ...
        QuerySlice::new(req.query()).map_or(
            ...
            |params| {
                let res = ArticleList::query_list_article(&req.state(), params.limit, params.offset, false);
                match res {
    ...
    fn list_comments(req: &HttpRequest<AppState>) -> JsonResponse {
        ...
        QuerySlice::new(req.query()).map_or(
            ...
            |params| {
                ...
                match Comments::query(&pg_pool, params.limit, params.offset, path_params.article_id) {
    ...
    fn view_article(req: &HttpRequest<AppState>) -> JsonResponse {
        ViewArticle::new(req.query()).map_or(
            api_resp_err!("'id' is not specified!"),
            |params| {
    ...
    fn login((req, params): (HttpRequest<AppState>, Form<LoginUser>)) -> JsonResponse {
        ...
        let max_age: Option<i64> = if is_remember {
            Some(24 * 90)
        } else {
            None
        };
...
//    }
    ...
    pub fn configure(app: App<AppState>) -> App<AppState> {
        app.scope("/article", |scope| {
            scope
                .resource("/view_all", |r| {
                    r.get().f(Visitor::list_all_article)
                })
                .resource("/view_all/{tag_id}", |r| {
                    r.get().f(Visitor::list_all_article_filter_by_tag)
                })
                .resource("/view_comment/{article_id}", |r| {
                    r.get().f(Visitor::list_comments)
                })
                .resource("/view/{article_id}", |r| {
