...
CREATE OR REPLACE FUNCTION page_num (idx bigint, page_size bigint) RETURNS bigint LANGUAGE PLPGSQL AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION comment_data (_article_id uuid, comment_id uuid, page_size bigint) RETURNS TABLE (j json) LANGUAGE plpgsql AS $$
BEGIN
    IF (
        SELECT
            count(id)
